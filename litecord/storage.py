...
class Storage:
    ...
    async def get_user(self, user_id, secure=False) -> Optional[Dict[str, Any]]:
        ...
        if secure:
            fields.extend(["email", "verified", "mfa_enabled", "nsfw_allowed"])
    ...
    async def get_attachments(self, message_id: int) -> List[Dict[str, Any]]:
        ...
        for attachment_id in attachment_ids:
            ...
            SELECT id::text, message_id, channel_id,
                   filename, filesize, image, height, width, content_type
            FROM attachments
            WHERE id = $1
            """,
                attachment_id,
            ...
            if drow["height"] is None and drow["width"] is None and drow["content_type"] == "video/mp4":
                drow["height"] = 720
                drow["width"] = 1280
                drow.pop("image")
            elif str(drow["content_type"]).startswith("video/"):
                drow["size"] = drow["filesize"]
                drow.pop("filesize")
                drow.pop("image")
