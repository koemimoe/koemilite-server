...
@bp.route("/banners/<int:guildoruser_id>/<icon_file>")
async def _get_banner(guildoruser_id: int, icon_file: str):
    is_guild = await app.db.fetchval(
            """
        SELECT id
        FROM guilds
        WHERE id = $1
    ...
    if is_guild:
        icon_hash, ext = splitext_(icon_file)
        return await send_icon("banner", guildoruser_id, icon_hash, ext=ext)
    else:
        icon_hash, ext = splitext_(icon_file)
        return await send_icon("userb", guildoruser_id, icon_hash, ext=ext)
