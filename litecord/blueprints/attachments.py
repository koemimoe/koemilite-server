...
async def _gen_img_from_vid(video, attach_id: int) -> str:
    """Generates an image from the video."""
    vimg_path = ATTACHMENTS / f"{attach_id}_video.jpg"
    vimg_path_s = str(vimg_path)
...
async def _get_attachment(channel_id: int, message_id: int, filename: str):
    ...
    if ext == "mp4":
        if request.args.get("format") and request.args.get("width") and request.args.get("height"):
            new_thumb = await _gen_img_from_vid(filepath, attach_id)
            return await send_file(new_thumb)
        else:
