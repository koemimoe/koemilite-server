    ...
    for overwrite in overwrites:
        ...
        await app.db.execute(
            ...
            INSERT INTO channel_overwrites
                ...
                target_user, allow, deny, guild_id)
            VALUES
                ($1, $2, $3, $4, $5, $6, $7)
        ...
        if target.is_user:
            ...
            await _dispatch_action(guild_id, channel_id, target.role_id, perms)

        elif target.is_role:
            assert target.user_id is not None
            user_ids.extend(await app.storage.get_role_members(target.user_id))
...
async def _update_category_channel(channel_id: int, j: dict, _user_id: int):
...
async def update_channel(channel_id: int):
    ...
    update_handler = {
        ...
        ChannelType.GUILD_CATEGORY: _update_category_channel,
    }[ctype]
