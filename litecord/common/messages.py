...
async def msg_add_attachment(message_id: int, channel_id: int, attachment_file) -> int:
    ...
    await app.db.execute(
        ...
        INSERT INTO attachments
            (id, channel_id, message_id,
             filename, filesize,
             image, width, height, content_type)
        VALUES
            ($1, $2, $3, $4, $5, $6, $7, $8, $9)
        """,
        attachment_id,
    ...
    if is_video:
        vidcap = cv.VideoCapture(f"attachments/{attachment_id}.{ext}")
        if vidcap.isOpened():
            width = int(vidcap.get(cv.CAP_PROP_FRAME_WIDTH))
            height = int(vidcap.get(cv.CAP_PROP_FRAME_HEIGHT))
            size = os.path.getsize(f"attachments/{attachment_id}.{ext}") 
            
        vidcap.release()
        
        await app.db.execute(
            """
            UPDATE attachments SET width = $2, height = $3, filesize = $4 WHERE id = $1
            """,
            attachment_id,
            width,
            height,
